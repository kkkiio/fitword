...
| 工具 | 职责 |
|---|---|
| `ask_question` | 展示题目 + 收集用户答案 + 选择题机械判定是否合格 |
...
选择题是确定性判定：Agent 提交题目、候选项和正确答案后，`ask_question` 可直接将用户选择与 `correct` 做机械比较并返回 `acceptance`。填空题和写作评分仍由 Agent 进行语义判断。
...
2. 等待用户作答。
3. 选择题将用户选择与 `correct` 做机械比较，得到 `acceptance`；填空题不判定。
...
| `acceptance` | `0` \| `1` | 是 | 二元合格判定。选择题来自 `ask_question` 的机械判定；填空题来自 Agent 的语义判断。`1` 表示合格，`0` 表示不合格；填空题答案能用但不够好时也可记为 `0` |
...
选择题:
  Agent → ask_question(question, candidates, correct) → 展示
        → 用户选 → 工具用 correct 机械判定 → 返回 { user_answer, acceptance }
        → record_answer(acceptance=0/1)
        → Agent 给选项辨析

填空题:
  Agent → ask_question(question) → 展示
        → 用户输入 → 返回 { user_answer }
        → Agent 语义判断 → record_answer(acceptance=0/1)
        → Agent 给评价反馈
```
...
{
  "weak_types": [
    { "knowledge_type": "adjective", "total": 10, "accepted": 4, "acceptance_rate": 40.0 }
  ],
  "overall": { "total_questions": 35, "acceptance_rate": 72.5 },
...
- 选择题是否合格由 `ask_question` 基于 Agent 提供的正确答案机械判定
