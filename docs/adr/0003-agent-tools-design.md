...
| 工具                 | 职责                                              |
| -------------------- | ------------------------------------------------- |
| `ask_question`       | 展示题目 + 收集用户答案；选择题机械判定并写入记录 |
| `record_answer`      | 写入填空题答题记录（含 Agent 判断的答题质量）     |
...
- 选择题由 `ask_question` 直接写入，因为它已经能机械判定 `quality`
- 填空题由 `record_answer` 写入，因为 Agent 需要在拿到用户输入后做语义判断
...
2. 等待用户作答。
3. 选择题将用户选择与 `correct` 做机械比较，得到 `quality`，并写入 SQLite。
4. 收集完成后以 tool result 形式返回。
5. 填空题不判定、不写存储，只返回用户答案。
...
## record_answer — 填空题答题记录工具

Agent 在获得填空题用户答案并判断答题质量后调用，写入 SQLite。
...
| `quality`        | `0` \| `1` \| `2` | 是   | Agent 对填空题答案的三档质量判断 |

### 行为

写入 SQLite `questions` 和 `answers` 表。两张表写入在同一事务中，因此只保存已经完成作答且 Agent 已判断质量的填空题。
...
| 方案                              | 优点                 | 缺点                                           |
| --------------------------------- | -------------------- | ---------------------------------------------- |
| 选择题也通过 record_answer 写存储 | 所有题型写库路径统一 | Agent 需要为机械判定题额外调用工具，增加出错点 |
| ask_question 对所有题型写存储     | 少一个工具           | 填空题无法在收集时判定质量，被迫用 NULL 占位   |
| 填空题传 expected/acceptable      | 工具可做自动判定     | 限制 Agent 灵活判断                            |
| 每个题型独立工具                  | 参数更精确           | 工具数量膨胀                                   |

## 后果

- 4 个工具，职责按能力边界拆分：展示/收集、填空题写入、展示评分、查询
- 选择题答题质量由 `ask_question` 基于 Agent 提供的正确答案机械判定，正确为 `2`，错误为 `0`，并由 `ask_question` 写入记录
