...
6. e2e 测试启动真实本地 Hono 服务。
7. e2e 测试使用独立临时 SQLite 数据库，避免污染用户本地数据。
8. 不维护 demo 降级路径。需要 LLM 的在线写作评分场景只有在 `.env` 或环境变量中存在 `OPENAI_API_KEY`、`OPENAI_BASE_URL`、`OPENAI_MODEL` 时启用，否则跳过。BDD 文本使用用户行为语言，模型服务细节保留在 fixture 和步骤实现中。
