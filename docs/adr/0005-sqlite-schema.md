...
| acceptance | INTEGER NOT NULL | 0 不合格 / 1 合格 |
| created_at | TEXT NOT NULL | ISO 8601 |
...
SELECT q.knowledge_type,
       COUNT(*) AS total,
       SUM(CASE WHEN a.acceptance = 1 THEN 1 ELSE 0 END) AS accepted,
       ROUND(SUM(CASE WHEN a.acceptance = 1 THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 1) AS acceptance_rate
FROM answers a JOIN questions q ON a.question_id = q.id
GROUP BY q.knowledge_type ORDER BY acceptance_rate ASC;
```
