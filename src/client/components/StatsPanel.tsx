    ...
    <div className="flex-1 overflow-auto p-6">
      <div className="mx-auto max-w-5xl space-y-6">
        <div>
          ...
          <p className="text-muted-foreground"><Trans>轻量概览本地练习记录，帮助你判断是否需要继续练习。</Trans></p>
        </div>

        <Card>
          <CardHeader>
            <CardTitle><Trans>答题概览</Trans></CardTitle>
            <CardDescription><Trans>答题数量与整体优质率。</Trans></CardDescription>
          </CardHeader>
          <CardContent>
            {!hasAnswers ? (
              <p className="text-sm text-muted-foreground"><Trans>暂无答题数据，完成练习后会显示概览。</Trans></p>
            ) : (
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  <div className="text-sm text-muted-foreground"><Trans>总题数</Trans></div>
                  <div className="text-3xl font-semibold">{stats.overall.total}</div>
                </div>
                <div className="space-y-2">
                  <div className="flex justify-between text-sm text-muted-foreground">
                    <span><Trans>优质率</Trans></span>
        ...
        <Card>
          <CardHeader>
            <CardTitle><Trans>选择 / 填空</Trans></CardTitle>
            <CardDescription><Trans>按题型展示题数和优质率。</Trans></CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {!hasAnswers ? <p className="text-sm text-muted-foreground"><Trans>暂无题型数据。</Trans></p> : null}
            {formatRows.map((row) => (
              <div key={row.format} className="rounded-lg border bg-background p-4">
                <div className="mb-3 flex items-center justify-between">
                  ...
                  <div className="text-sm text-muted-foreground"><Trans>{row.total} 题</Trans></div>
                </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription><Trans>展示五维平均分。</Trans></CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {!hasWriting ? (
              <p className="text-sm text-muted-foreground"><Trans>暂无写作评分数据，完成评分后会显示五维平均分。</Trans></p>
            ) : (
              <>
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="rounded-lg border bg-background p-3">
                    <div className="text-sm text-muted-foreground"><Trans>评分次数</Trans></div>
                    <div className="text-2xl font-semibold">{writing.total_records}</div>
                  </div>
                  <div className="rounded-lg border bg-background p-3">
                    <div className="text-sm text-muted-foreground"><Trans>平均总分</Trans></div>
